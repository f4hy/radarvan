...
`player_synergy` still filters on `.ratings` and drops any game containing a sub-45-game player — that one is a deliberate methodology choice, not the same bug.

The prior itself is **relative and re-applied per pass**, and both halves are load-bearing. Relative — one beta below the weakest established player, at twice the usual sigma — because openskill has no absolute anchor: this corpus has drifted to a mu range of ~9–37, so the library default of `mu=25` seeded a brand-new player above two thirds of the regulars, and lowering the default *for everyone* was measured to shift every mu down by the same amount and leave the ordering byte-identical. Re-applied because `compute_player_ratings` runs `ITERATIONS = 3` passes and feeds each one the *previous pass's output*, so a starting prior only constrains pass 1: seeding a 36-game newcomer 25 mu lower moved them 8th → 13th after one pass and left them 5th after three. `_reseed` is where it lands, shrinking mu toward the prior with a weight that ramps linearly to 1.0 at `MIN_GAMES` (continuous on purpose — a threshold would step a player's rating the night they crossed it, and at full trust it reduces exactly to the sigma floor it replaced). **Only mu is pulled back.** In openskill sigma *is* the learning rate, so re-inflating a newcomer's uncertainty each pass pins them at maximum volatility and lets a short winning run carry them further than the evidence does — the opposite of what the prior is for.

**Guest ratings are asserted, not learned.** `player_rating.GUEST_RATING_MULTIPLIERS` (`Excal` 2.0, `Domi` 1.5) overwrites those two players' ratings with that multiple of the top established human's ordinal, at `GUEST_SIGMA`. They are (semi-)pro visitors: everything above is designed to stop a handful of games minting a top rating, which is right for a stranger and wrong for someone already known to outclass the group. **Applied as the last step of `compute_player_ratings`, after `_newcomer_prior`, and never during the passes** — rating the group's real games against an asserted 2x opponent would mean losing to Excal barely dents anyone. A *multiple of the leader* rather than a fixed mu for the same reason the newcomer prior is relative. The blast radius is deliberately one number: it reaches `rating_for` (so `create_teams.rate_roster` and `ml/predict` see it) and nothing else — the guests are under `MIN_GAMES`, so the fabricated level never reaches `.ratings` or the wire. Fake *games* were the alternative and are much worse: a match is a row in the one corpus that records, head-to-head, superlatives, the recap and the duration histogram all read. `tests/test_guest_ratings.py` pins the radius; delete a dict entry to hand the player back to the model. Note they are otherwise ordinary members of `PLAYER_NAMES` — their real games rate and count for records like anyone's.
