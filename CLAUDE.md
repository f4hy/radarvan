...
**Player roles — never re-derive them.** Whether a slot is a human, an AI, or a spectator comes from the replay header (`type == "C"` for AI; a spectator is a type-`"H"` slot with `playerTemplate` `-2`) and is persisted as `match_players.role` (`player_role.PlayerRole`). Do **not** write a new observer/CPU check: build a `game_composition.MatchRoster` (`from_db_players` / `from_players`) and ask it — `.humans`, `.cpus`, `.observers`, `.competitors` (played, teamless included), `.participants` (competitors with `team > 0`), `.teams`. The old codebase spelled "skip the observers" three inconsistent ways (`team > 0`, `team == Team.OBSERVER`, `is_real()`) and carried two disagreeing CPU-name lists, which counted AI slots named outside a five-entry list (Tactical AI, EasyArmy, MediumArmy — 417 matches) as humans. Guessing from the player's *name* is the fallback for un-backfilled rows only (`player_role.resolve_role`); the header is authoritative and an AI's header name is empty. `role` is still nullable — tighten to NOT NULL once `list_matches_with_unset_roles` returns empty.
