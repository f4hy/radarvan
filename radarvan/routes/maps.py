...
async def push_maps_to_cncstats(
    ...
    before pushing - so a map is never sent twice. Processes up to `max_to_update`
    unsynced maps. Requires `CNCSTATS_API_KEY`. The scheduler runs the same pass
    (see `schedule.sync_maps_to_cncstats`); this route is for forcing it now.
    """
